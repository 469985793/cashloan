...
@Component
@Lazy(value = false)
public class QuartzRepaymentIntraday implements Job {

	private static final Logger logger = Logger.getLogger(QuartzRepaymentIntraday.class);


	private String repayment() throws ServiceException {
		logger.info("进入-还款日-代扣还款任务...");
		...
		for (BorrowRepay borrowRepay : borrowRepayList) {
			...
			try {
				...
				PayLog repaymentLog = payLogService.findRepayLatestOne(borrowRepay.getUserId(), borrowRepay.getBorrowId());
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		// 查询当前任务信息
		QuartzInfo quartzInfo = quartzInfoService.findByCode("doRepaymentIntraday");
